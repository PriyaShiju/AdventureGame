...
function StoryGenerator()
{
    ...
    const generateStory =async(theme) => {
        ...
        setTheme(theme)
        try{
            const response = await axios.get(`${API_BASE_URL}/stories/stories`,{theme})
    ...
    const pollJobStatus = async(id) => {
        try{
            const repsone=await axios.get(`${API_BASE_URL}/jobs/${id}`)
    ...
    const fetchStory = async(id) => {
        try{
            ...
            setError(`Failed to load story: ${e.messages}`)
            setLoading(false)
        }
}
    const reset =()=>{
